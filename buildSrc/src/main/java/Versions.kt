object Versions {

    // App data
    const val MIN_SDK_VERSION = 21
    const val COMPILE_SDK_VERSION = 28
    const val TARGET_SDK_VERSION = 28
    const val VERSION_CODE = 1
    const val VERSION_NAME = "0.0.1"

    // Core
    const val ADAPTER_DELEGATES_VERSION = "4.1.1"
    const val DAGGER_VERSION = "2.24"
    const val RETROFIT_VERSION = "2.6.1"
    const val MOSHI_VERSION = "1.8.0"
    const val OKHTTP_VERSION = "4.1.0"
    const val THREE_TEN_ABP_VERSION = "1.2.1"

    // Kotlin
    const val KOTLIN_VERSION = "1.3.50"
    const val KOTLIN_COROUTINES_VERSION = "1.3.0"

    // Plugins
    const val GRADLE_ANDROID_VERSION = "3.4.0"
    const val DETEKT_VERSION = "1.0.0"
    const val GRADLE_VERSIONS_VERSION = "0.22.0"
    const val SAFE_ARGS_VERSION = "1.0.0-alpha06"
    const val ANDROID_JUNIT5_VERSION = "1.5.1.0"
    const val DEV_METRICS_VERSION = "0.7"

    // JetPack - AppCompat
    const val APPCOMPAT_VERSION = "1.1.0-rc01"
    const val MATERIAL_COMPONENTS_VERSION = "1.1.0-alpha09"
    const val CONSTRAINT_LAYOUT_VERSION = "2.0.0-beta2"
    const val LEGACY_SUPPORT_UTILS_VERSION = "1.0.0"

    // JetPack - Architecture
    const val LIFECYCLE_VERSION = "2.2.0-alpha03"
    const val ROOM_VERSION = "2.2.0-beta01"
    const val NAVIGATION_VERSION = "2.2.0-alpha01"

    // JetPack - KTX
    const val KTX_VERSION = "1.2.0-alpha03"

    // UI
    const val MATERIAL_VALUES_VERSION = "1.1.1"
    const val MATERIAL_DIALOGS_VERSION = "3.1.0"
    const val MP_ANDROID_CHART_VERSION = "v3.0.3"
    const val SHAPE_OF_VIEW_VERSION = "1.4.7"

    // Testing
    const val JUNIT5_VERSION = "5.2.0"
    const val JUNIT5_RUNNER_VERSION = "0.2.2"
    const val TEST_RUNNER_VERSION = "1.3.0-alpha02"
    const val TEST_RULES_VERSION = "1.1.0-alpha1"
    const val ESPRESSO_VERSION = "3.1.0-alpha1"
    const val SPEK_VERSION = "2.0.6"
    const val EXPEKT_VERSION = "0.5.0"
    const val KAKAO_VERSION = "1.3.0"
    const val FUEL_VERSION = "2.2.0"

    // Development tools
    const val TIMBER_VERSION = "4.7.1"
    const val LEAK_CANARY_VERSION = "2.0-beta-3"
    const val ANDROID_DEBUG_DATABASE_VERSION = "1.0.6"
}
